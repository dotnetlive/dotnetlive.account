...
namespace DotNetLive.Framework.Dapper.Extensions
{
    public static partial class SqlMapperExtensions
    {
        ...
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, IEnumerable<PropertyInfo>> GeneratedProperties = new ConcurrentDictionary<RuntimeTypeHandle, IEnumerable<PropertyInfo>>();
        ...
        private static List<PropertyInfo> GeneratedPropertiesCache(Type type)
        {
            IEnumerable<PropertyInfo> pi;
            if (GeneratedProperties.TryGetValue(type.TypeHandle, out pi))
            {
                return pi.ToList();
            }

            var generatedProperties = TypePropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is DatabaseGeneratedAttribute)).ToList();

            ComputedProperties[type.TypeHandle] = generatedProperties;
        ...
        private static List<PropertyInfo> TypePropertiesCache(Type type)
        {
            ...
            var properties = type.GetProperties().Where(x => IsWriteable(x) && HasColumnAttribute(x)).ToArray();
        ...
        private static bool HasColumnAttribute(PropertyInfo pi)
        {
            return pi.GetCustomAttributes(typeof(ColumnAttribute), false).Any();
        }
        ...
        public static TKey Insert<T, TKey>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var generatedProperties = GeneratedPropertiesCache(type);
            //var allPropertiesExceptKeyAndComputed = allProperties.Except(keyProperties.Union(computedProperties)).ToList();
            var allPropertiesExceptComputed = allProperties.Except(computedProperties.Union(generatedProperties)).ToList();
            ...
            {
                returnVal = adapter.Insert<TKey>(connection, transaction, commandTimeout, name, sbColumnList.ToString(),
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var computedProperties = ComputedPropertiesCache(type);
            var generatedProperties = GeneratedPropertiesCache(type);
            var nonIdProps = allProperties.Except(keyProperties.Union(computedProperties).Union(generatedProperties)).ToList();
